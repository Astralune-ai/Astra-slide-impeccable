...
def order_from_viewer(index_path):
    """Read the playback order out of an index.html viewer.

    The viewer declares its own order as `const slides = [['S03-x.html', '…'], …]`.
    Filename sort is NOT that order once a deck has inserts (S00b-, S03b-) or a
    page has been moved (thesis S01 shown *after* market S02) — and a wrong page
    order exports silently, which is the dangerous kind. Trust the viewer.

    Returns absolute paths in declared order, or None if it can't be parsed."""
    try:
    ...
    if not m:
        return None
    # first quoted string of each row = the html filename
    ...
    for n in names:
        ...
        (files if os.path.isfile(p) else missing).append(p if os.path.isfile(p) else n)
    if missing:
        print(f'  ! viewer lists {len(missing)} missing file(s): {", ".join(missing[:3])}'
              f'{"…" if len(missing) > 3 else ""} — falling back to filename order')
        return None
...
def find_deck_files(input_path):
    ...
    per-slide HTML files. Playback order from index.html wins over filename sort."""
    base = None
    ...
    # 1) viewer-declared order (authoritative — survives inserts and reordering)
    for cand in ('index.html', 'index.htm'):
        idx = os.path.join(base, cand)
        if os.path.isfile(idx):
            ordered = order_from_viewer(idx)
            if ordered:
                print(f'  · order from {cand} ({len(ordered)} slides)')
                return ordered
            break

    # 2) fall back to filename sort
    ...
    if files:
        print(f'  · order from filename sort ({len(files)} slides) '
              f'— no parsable index.html; verify page order in the PDF')
    return files or None
