...
**Vibe:** Scholar-engineer, quietly ambitious, AI-era Chinese corporate. Warm parchment × sage teal × engineering blueprint — the bilingual business pitch aesthetic for founders who read both 宋代书画 and technical white papers. First-person-plural confidence without noise.

**Origin:** Distilled from the 企合智汇 · Astra 31-slide deck (2026-04-18) used to win an enterprise AI deployment engagement. Battle-tested on a 18-min founder pitch to a Chinese-Australian C-suite client.

**Layout:** 1400×788 SVG viewBox canvas. Generous margins (40-80px edges). Content either left-anchored with a right-side ornament (cover pages) or horizontally balanced (comparison/matrix/cards). Every slide has a light engineering grid underlay — the content floats on blueprint paper.

**Typography:**
- Display: `Noto Serif SC` (900) — 宋体 black weight for Chinese titles
- Body: `Noto Sans SC` (400/500/700) — Chinese body & UI text
- Micro/label: `JetBrains Mono` (400/500/600) — English engineering labels with 2-5px letter-spacing
- Accent italic: `Instrument Serif` (italic) — English subtitles in elegant italic serif
- Numeric display: `Space Grotesk` (700/900) — for big stat numbers, Q1/Q2 badges, bar percentages

**Colors:**
```css
:root {
  --bg: #f4f1e8;          /* 素白宣纸 · parchment cream */
  --ink: #1a1714;         /* 徽墨 · near-black ink */
  --ink-soft: #4a433c;    /* 淡墨 · soft ink, body text */
  --warm-grey: #7a7268;   /* 温灰 · muted labels, captions */
  --sage: #3a9687;        /* 青竹绿 · signature sage teal */
  --vermilion: #a03a36;   /* 朱砂 · warning / key spotlight */
  --indigo: #25577a;      /* 靛青 · secondary accent */
  --copper: #c88763;      /* 古铜 · tertiary (trap/caution) */
  --leaf: #3d7a50;        /* 叶绿 · included/safe */
}
```

**Signature Elements:**
- **Dual-layer grid pattern underlay** (20px fine + 100px heavy) at 18-22% opacity — engineering-blueprint feel without dominating
- **Corner ring** top-right: 3 nested circles (r=22/16/9), inner circle with `stroke-dasharray="3,4"` slow-rotating 360°/22s, centered page number
- **Blade accent line** under titles: short horizontal stroke-width:1.4-2 sage line, often animated extending from 0 → 60-120px width
- **Ornament orb** (cover/hero only): radial halo gradient + concentric dashed rings rotating at different speeds (counter-rotating) + center white circle + Chinese motif character (启/承/转/合/道/志) at 56-72px in sage
- **Satellite dots** — 2-3 small circles (r=2.5-3) in vermilion/indigo/sage with perpetual sine-ease opacity pulse
- **`x`/`y` SVG text layout** (not flexbox) — every element has precise coordinate → maps directly to design handoff
- **Header microlabel** top-left: `JetBrains Mono` 9px with 2px letter-spacing, sage at 55-60% opacity (e.g., `IMPECCABLE · v2 · Astra`)
- **Footer doubled microlabels** bottom corners: page ID + thematic tagline in matching sage 10px
- **Color-coded quadrants** in 2×2 matrices: sage = champion, indigo = respected expert, warm grey = at-risk, copper = trap
- **`filter="url(#softGlow)"`** — 5-stdDev Gaussian blur with 250% filter region (x="-75%" y="-75%" width="250%" height="250%") on hero numbers and champion borders; never square filter region bugs
- **Animated number counters** — `{v:0}` → target via GSAP power2.out + `toLocaleString('en-US')` for $ figures
- **Card 3px/4px left spine** in accent color (sage for champion, vermilion for warning, warm grey for neutral)

**Animation choreography:**
- Slide-internal timeline with `gsap.timeline({ defaults: { ease: 'power3.out' } })`
- 0.0-0.3s: microlabel fade in
- 0.3-0.8s: title + subtitle stagger
- 0.8-1.5s: main panels slide in from side or stagger from center
- 1.5-2.5s: accent elements (VS badge bounce, insight bar rise, champion halo breath)
- Perpetual: inner ring rotation, satellite dot pulse, champion border breath via `strokeWidth` yoyo
- **NEVER** tween `filter: brightness()` from 0 — use `strokeOpacity` or `opacity` breath instead (per hard rule 8)

**When to use:**
- Chinese-audience B2B pitches (enterprise AI, consulting, SaaS deployment)
- Founder pitches where you want to read as "儒商精工" (scholar-craftsman) not Silicon Valley loud
- AI / automation deployment proposals to Chinese-speaking C-suite
- Bilingual decks where CJK titles + English micro-labels coexist naturally
- Any context where engineering rigor + editorial warmth needs to coexist
- Specifically NOT for: consumer brands, entertainment, creative agency showcase (too restrained)

**Layout DNA:**
- Slide mechanism: each slide = full SVG `viewBox="0 0 1400 788"` inside `.slide { position:fixed; inset:0; opacity:0 }` wrapper
- Background: parchment cream (#f4f1e8) + dual grid pattern + optional radial halo from ornament
- Accent: sage teal (#3a9687) — used for titles, blades, rings, satellite dots, champion borders
- Typography: Serif SC 900 for titles, JetBrains Mono for every English micro-label
- Signature motif: character-centered orb (启/承/转/合) on cover, precise SVG coordinate system throughout
- Canonical sample: `styles/qihe-sage.html` — 5-slide demo (cover · 2×2 matrix · comparison · 3-card promises · close)
